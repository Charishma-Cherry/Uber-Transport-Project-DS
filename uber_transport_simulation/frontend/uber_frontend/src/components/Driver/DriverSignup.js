...
function DriverSignup() {
  ...
  const [introductionMedia, setIntroductionMedia] = useState(null);
  const [previewMedia, setPreviewMedia] = useState(null); // For previewing image or video
  const [error, setError] = useState('');
  const [loading, setLoading] = useState(false);
  const navigate = useNavigate();
  ...
  const handleFileChange = (e) => {
    ...
    if (!validTypes.includes(file.type)) {
      setError('Only JPG, PNG images or MP4 videos are allowed.');
      return;
    }
            ...
            <Form.Group className="form-column mb-3">
              ...
              {previewMedia && (
                <>
                  ...
                  {introductionMedia?.type.startsWith('video') && (
                    <video
                      controls
                      style={{ width: '100%', marginTop: '10px' }}
                    >
