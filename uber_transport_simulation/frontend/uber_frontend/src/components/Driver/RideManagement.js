  ...
  const RideManagement = () => {
    ...
    const navigate = useNavigate(); // Correctly use useNavigate at the top level
    ...
    useEffect(() => {
      const loadRides = async () => {
        ...
        try {
          ...
          // Filter out rides with the status 'completed'
          const filteredRides = data.filter((ride) => ride.status !== 'completed');
          console.log('Filtered rides (excluding completed):', filteredRides);
  
          setRides(filteredRides); // Update state with filtered rides
        } catch (error) {
    ...
    const handleStatusChange = async (rideId, newStatus) => {
      ...
      try {
        ...
        setRides((prevRides) =>
          ...
          ).filter((ride) => ride.status !== 'completed') // Remove completed rides
        );
    
        alert('Ride status updated successfully.');
    
        // Redirect to the rating page if the status is 'completed'
        if (newStatus === 'completed') {
          console.log('Navigating to rate-user page with rideId:', rideId);
          navigate(`/rate-user/${rideId}`); // Navigate to the RateUser page
        }
      } catch (error) {
