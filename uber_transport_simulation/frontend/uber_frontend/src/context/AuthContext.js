...
export const AuthProvider = ({ children }) => {
  ...
  const loginAdmin = async (username, password) => {
    try {
      const response = await api.post(endpoints.ADMIN_LOGIN, { username, password });
      const { token, user } = response.data;
      ...
      // Save admin token and user info to localStorage
      localStorage.setItem('token', token);
      localStorage.setItem('user', JSON.stringify(user));
      localStorage.setItem('userType', 'admin'); // Set userType to 'admin'

      api.defaults.headers.common['Authorization'] = `Token ${token}`;

      setUser(user);
      console.log('Admin login successful, user set:', user);
      return true;
    } catch (error) {
  ...
  return (
    <AuthContext.Provider value={{ user, loading, login, logout, fetchUser, loginDriver, loginAdmin }}>
      {children}
